...
<h1 class="text-5xl font-bold text-center my-10">Sign up</h1>
...
<div class="mx-3 flex flex-col justify-center items-center">
	...
	>
		...
		<label class="form-control w-full my-3">
			...
			<input
				...
				placeholder="********"
				class="input input-bordered w-full"
				class:input-error={form?.ctx === 'confirm_password' ||
					form?.ctx === 'global'}
			/>
			{#if form?.ctx === 'confirm_password'}
				<div class="label">
					<span class="label-text-alt text-error">{form.error}</span>
				</div>
			{/if}
	...
	<div class="mt-3">
		...
		<a href="/login" class="link link-hover text-[blue]">Login</a>
	</div>
</div>
